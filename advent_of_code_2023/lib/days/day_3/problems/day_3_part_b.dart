...
class Day3PartB extends Day3Problem {
  const Day3PartB({required super.day3repo});
  ...
  Day3PartBSetNode? _getSetNode({
    ...
    if (willCheckTop) {
      ...
      final points = _doesLineHaveSymbolInRange(
          model: model, yLoc: yLoc, xStart: xStart, xFinish: xFinish);
      assignPoints(points);
      final node =
          buildDay3PartBSetNode(node1Loc: node1Loc, node2Loc: node2Loc);
      if (node != null) {
        return node;
      }
    }
